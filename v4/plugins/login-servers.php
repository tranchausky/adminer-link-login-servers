...
//include_once __DIR__. '/_config-server.php';
  ...
  */
  function __construct() {
    ...
    if ($_ENV['ADMINER_SERVERS']) {
	  ...
	  $list = json_decode($_ENV['ADMINER_SERVERS'], true);
	
		//var_dump($list);
		
		$list["Server 4"] = [
			"driver" =>"server",
			"server"=>"192.168.1.104:3307",
			"username"=>"r1concept2",
			"password"=>"r1concept2",
			"db"=>""
		];
		...
		//var_dump($list);
		  ...
		  $this->servers = $list;
		  $fileServers = json_decode(
				file_get_contents(__DIR__ . '/_config-server.json'),
				true
			) ?: [];
		  //$fileServers = include __DIR__ . '/_config-server.php';
		  $this->servers = array_merge($list, $fileServers);
    }
	//var_dump($this->servers);die;
